...
fn handleLint(base_allocator: std.mem.Allocator, args: []const []const u8) u8 {
    ...
    var diags: std.ArrayListUnmanaged(LintDiagnostic) = .{};
    defer diags.deinit(base_allocator);

    for (resolved.items) |path| {
        if (!lintFile(base_allocator, path, &diags, err_writer)) return 2;
    }

    if (diags.items.len == 0) return 0;

    for (diags.items) |diag| {
        err_writer.print("{s}:{d}:{d}: warning: [{s}] {s}\n", .{
            diag.file,
            diag.line,
            diag.column,
            diag.rule_id,
...
fn lintFile(allocator: std.mem.Allocator, path: []const u8, diags: *std.ArrayListUnmanaged(LintDiagnostic), err_writer: anytype) bool {
    const source = std.fs.cwd().readFileAlloc(allocator, path, 16 * 1024 * 1024) catch |err| {
        err_writer.print("Error: cannot read '{s}': {any}\n", .{ path, err }) catch {};
        err_writer.flush() catch {};
        return false;
    };
    ...
    var tokenizer = Tokenizer.initForFormatting(source);
    ...
    lintOrphanedDocComment(path, tokens.items, diags, allocator) catch return oomLint(err_writer);
    lintDocCommentMisuse(path, tokens.items, diags, allocator) catch return oomLint(err_writer);
    lintNonKebabCase(path, tokens.items, diags, allocator) catch return oomLint(err_writer);
    lintCommentMissingSpace(path, tokens.items, diags, allocator) catch return oomLint(err_writer);
    lintExcessiveBlankLines(path, source, diags, allocator) catch return oomLint(err_writer);
    return true;
}

fn oomLint(err_writer: anytype) bool {
    err_writer.writeAll("Error: out of memory\n") catch {};
    err_writer.flush() catch {};
    return false;
}

fn appendLintDiag(diags: *std.ArrayListUnmanaged(LintDiagnostic), allocator: std.mem.Allocator, file: []const u8, line: usize, column: usize, rule_id: []const u8, message: []const u8) !void {
...
fn isWord(tok: Token, text: []const u8) bool {
    return tok.kind == .word and std.mem.eql(u8, tok.text, text);
}

fn isDefinitionToken(tok: Token) bool {
    return tok.kind == .word and std.mem.endsWith(u8, tok.text, ":");
}

fn updateStatementStart(tok: Token, started: bool) bool {
    if (tok.kind == .comment or tok.kind == .doc_comment or tok.kind == .newline) return started;
    if (isWord(tok, ";") or isWord(tok, "[")) return true;
    return false;
}

fn lintOrphanedDocComment(file: []const u8, tokens: []const Token, diags: *std.ArrayListUnmanaged(LintDiagnostic), allocator: std.mem.Allocator) !void {
    ...
    for (tokens) |tok| {
        switch (tok.kind) {
            .doc_comment => {
                if (pending == null) pending = tok;
            },
            .comment, .newline => {},
            .word => {
                if (pending) |doc| {
                    if (!(statement_start and isDefinitionToken(tok))) {
                        try appendLintDiag(diags, allocator, file, doc.line, doc.column, "orphaned-doc-comment", "doc comment not followed by a definition");
                    }
                    pending = null;
                }
                statement_start = updateStatementStart(tok, statement_start);
            },
        }
    }
...
fn lintDocCommentMisuse(file: []const u8, tokens: []const Token, diags: *std.ArrayListUnmanaged(LintDiagnostic), allocator: std.mem.Allocator) !void {
    var bracket_depth: usize = 0;
    var line_has_code = false;

    for (tokens) |tok| {
        switch (tok.kind) {
            .newline => line_has_code = false,
            .comment => {},
            .doc_comment => {
                if (bracket_depth > 0 or line_has_code) {
                    try appendLintDiag(diags, allocator, file, tok.line, tok.column, "doc-comment-misuse", "doc comment used where regular comment would suffice");
                }
            },
            .word => {
                if (isWord(tok, "[")) {
...
fn trimStandardAffixes(name: []const u8) []const u8 {
    var out = name;
    while (out.len > 0 and (out[0] == '#' or out[0] == '@' or out[0] == '>' or out[0] == '<')) {
        out = out[1..];
    }
...
fn isNonKebabCase(name: []const u8) bool {
    const core = trimStandardAffixes(name);
    if (core.len == 0) return false;

    var has_letter = false;
    for (core) |ch| {
        if (isAsciiLower(ch) or isAsciiUpper(ch)) has_letter = true;
        if (!(isAsciiLower(ch) or isAsciiUpper(ch) or isAsciiDigit(ch) or ch == '-' or ch == '_')) {
            return false;
        }
    }
    if (!has_letter) return false;

    if (core[0] == '-' or core[core.len - 1] == '-') return true;
    if (std.mem.indexOf(u8, core, "--") != null) return true;

    for (core) |ch| {
        if (!(isAsciiLower(ch) or isAsciiDigit(ch) or ch == '-')) return true;
    }
    return false;
}

fn lintNonKebabCase(file: []const u8, tokens: []const Token, diags: *std.ArrayListUnmanaged(LintDiagnostic), allocator: std.mem.Allocator) !void {
    var statement_start = true;
    ...
    for (tokens) |tok| {
        if (tok.kind == .word) {
            if (statement_start and isDefinitionToken(tok)) {
                ...
                if (isNonKebabCase(name)) {
                    try appendLintDiag(diags, allocator, file, tok.line, tok.column, "non-kebab-case", "definition name is not kebab-case");
                }
            }
            statement_start = updateStatementStart(tok, statement_start);
        }
...
fn lintCommentMissingSpace(file: []const u8, tokens: []const Token, diags: *std.ArrayListUnmanaged(LintDiagnostic), allocator: std.mem.Allocator) !void {
    for (tokens) |tok| {
        if (tok.kind == .word and tok.text.len > 0 and tok.text[0] == '\\') {
            try appendLintDiag(diags, allocator, file, tok.line, tok.column, "comment-missing-space", "comment marker not followed by a space");
        }
    }
}
...
fn isBlankLine(line: []const u8) bool {
    for (line) |ch| {
        if (!(ch == ' ' or ch == '\t' or ch == '\r')) return false;
    }
    return true;
}

fn lintExcessiveBlankLines(file: []const u8, source: []const u8, diags: *std.ArrayListUnmanaged(LintDiagnostic), allocator: std.mem.Allocator) !void {
    var line_no: usize = 1;
    var start: usize = 0;
    ...
    while (i <= source.len) : (i += 1) {
        ...
        if (isBlankLine(line)) {
            blank_run += 1;
            if (blank_run == 3) {
                try appendLintDiag(diags, allocator, file, line_no, 1, "excessive-blank-lines", "3 or more consecutive blank lines");
            }
        } else {
            blank_run = 0;
        }
