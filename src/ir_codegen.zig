...
const ValueLayout = struct {
    ...
    fn payloadKindOf(tag: TagType) PayloadKind {
        return switch (tag) {
            ...
            .float => .f64_,
            .boolean => .bool_,
            .hash, .vector, .byte_array, .set, .mutable_map, .array, .stream, .resource, .parameter, .module, .marker, .struct_type, .struct_instance, .benchmark_report, .task, .channel, .iterator, .type_val, .sandbox_spec, .error_value, .bignum => .ptr,
