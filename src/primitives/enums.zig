...
pub const primitives = [_]Primitive{
    .{ .name = "define-enum", .stack_effect = "name: descriptor markers --", .doc = "Define an enum type with flat variant constructors and predicates.", .func = nativeDefineEnum, .defines_word = true },
...
fn nativeDefineEnum(ctx: *Context) anyerror!void {
    ...
    // Every word below is generated by this declaration, including the per-variant words the
    // shared `virtual` helpers define, so the site is published on the context rather than
    // threaded through each definition call.
