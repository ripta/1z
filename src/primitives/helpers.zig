...
pub fn valueTypeName(val: Value) []const u8 {
    return switch (val) {
        ...
        .value_map => "value-map",
        .mutable_value_map => "mutable-value-map",
        .stream => "stream",
        .resource => "resource",
        .parameter => "parameter",
        .module => "module",
...
pub fn formatValueBrief(allocator: Allocator, val: Value, max_len: usize) ![]const u8 {
    return switch (val) {
        ...
        .value_map => |m| std.fmt.allocPrint(allocator, "value-map[{d}]", .{m.map.count()}),
        .mutable_value_map => |m| std.fmt.allocPrint(allocator, "mutable-value-map[{d}]", .{m.map.count()}),
