...
pub const primitives = [_]Primitive{
    .{ .name = "define-struct", .stack_effect = "name: descriptor markers --", .doc = "Define a struct type and its accessor words.", .func = nativeDefineStruct, .defines_word = true },
};
...
fn nativeDefineStruct(ctx: *Context) anyerror!void {
    ...
    // Every word below is generated by this declaration, including the ones the shared
    // `virtual` helpers define, so the site is published on the context rather than threaded
    // through each definition call.
    const saved_src_loc = ctx.generated_src_loc;
    defer ctx.generated_src_loc = saved_src_loc;
    ctx.generated_src_loc = try helpers.genSrcLocFrom(alloc, raw_desc_map.map.get("src-loc"));
