...
aot-checks: aot-interpreter-strip-check aot-line-directives-check aot-asm-name-check aot-string-literal-direct-check aot-symbol-literal-direct-check aot-trace-instr-check aot-trace-word-filter-check aot-param-inference-check aot-determinism-check ## Run the AOT compiler-behavior checks
...
benchmark-quotation-bracket: build ## A/B what the transient lexical frame costs a flagged quotation body, per path and mode
	@scripts/benchmark-quotation-bracket.sh ./$(ZIG_PREFIX)/bin/1z $(ZIG_PREFIX) 7 \
		"splice|tests/benchmark/quotation_bracket_splice_bare.1z|tests/benchmark/quotation_bracket_splice_flagged.1z" \
		"per-element|tests/benchmark/quotation_bracket_each_bare.1z|tests/benchmark/quotation_bracket_each_flagged.1z" \
		> tests/benchmark/quotation_bracket.sample
	@cat tests/benchmark/quotation_bracket.sample
...
benchmark-stmt: build ## Run the hosted statement benchmark and record the sample
	./$(ZIG_PREFIX)/bin/1z run tests/benchmark/stmt_pipeline.1z > tests/benchmark/stmt_pipeline.sample
	@cat tests/benchmark/stmt_pipeline.sample
...
wasm: ## Build the wasm32-freestanding browser module and copy it into examples/wasm-repl/, examples/wasm-game/, examples/wasm-snake/, and examples/wasm-minesweeper/
	...
	cp $(ZIG_PREFIX)/wasm/1z.wasm examples/wasm-minesweeper/1z.wasm
	@echo "PASS: 1z.wasm built and copied into examples/wasm-repl/, examples/wasm-game/, examples/wasm-snake/, and examples/wasm-minesweeper/"
...
wasm-minesweeper-verify: wasm ## Headlessly verify minesweeper against the wasm build via Node (not part of make test)
