#!/usr/bin/env bash
#
# Run scanner vs direct benchmark across interpreter execution modes and
# print a comparison.
#
# Usage: scripts/benchmark-scanner.sh <1z-binary> <benchmark-file>
#
# The script runs the benchmark in interpreted, hybrid, and eager modes,
# then prints a timing breakdown table. AOT is skipped because the
# benchmark uses library imports (scanner, strings) which the AOT
...
echo ""
echo "(AOT skipped: benchmark uses library imports not yet supported by AOT compiler)"
