...
Neither engine writes a file whose text already matches its formatted form, so
formatting a clean tree leaves every modification time alone.

A file that does change is replaced rather than overwritten. Both engines write
the formatted text to a temporary beside the file and rename it over the target.
Another program reading the file at that moment sees the old text or the new
text, never a half-written one.

Three consequences follow. The file's inode changes, so an advisory lock, a
descriptor held open elsewhere, or a file watch keyed on the old inode stops
referring to what is at that path. The containing directory has to be writable,
because the replacement creates an entry in it. And a file carrying more than
one hard link is refused, since a rename breaks a link by construction. That
file is named and left alone, and the run continues with the rest.

A rewrite that would leave the file empty is refused the same way. A file
holding only whitespace is the shape this reaches: every token is dropped, so
its formatted form is nothing, and writing that destroys content no later run
can recover. The file is named on standard error, it keeps its contents, and the
run exits 1. Normalizing such a file down to empty is what this gives up.

The refusal is on the write alone. `format-source` and `format-file` still
answer the empty string for whitespace-only text, and `1z fmt --stdout` still
